...
public class SectionResource {
	...
	public Response addSectionToProfile(@PathParam("profileId") String profileId, String sectionNameJson) throws SQLException {
		Gson gson = new Gson(); //Instancie un objet JSON
		Section section = gson.fromJson(sectionNameJson, Section.class); //Permet de créer un objet Seczion avec le JSON
		//Création de la requête permettant de sélectionner les informations de la section
		String requestSectionId = "SELECT * FROM section WHERE name='" + section.getName() + "';";
		//Création d'une section suivant les informations récupérées par la requête précédente
		Section sectionFull = SqlRequest.requestSection(requestSectionId);
		//Création de la requête permettant de lier la section au profil
		String requestBind = "INSERT INTO profile_section_junction VALUES(" + profileId + ", " + sectionFull.getId() + ");"; 
		//Transmet la requête à la classe JDBC
